...
def render_page(text, tokens_text, embeddings):
    ...
    # Add input box for custom words
    custom_words = st.text_input("Enter custom words (comma-separated)").split(',')

    # Highlight custom words
    custom_indices = [i for i, token in enumerate(tokens) if token.lower() in [word.strip().lower() for word in custom_words]]
    for idx in custom_indices:
        df.at[idx, "Label"] = "Custom"
    ...
    for category in ["Category 1", "Category 2", "Category 3", "Category 4", "Category 5", "Category 6", "Custom"]:
